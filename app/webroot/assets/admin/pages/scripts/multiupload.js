		...
		if(this.items.length > 0){
			...
			var uId = "";
 			...
 			for(var i = 0; i<this.items.length; i++){
				...
				var sampleIcon = '<img src="'+SITE_URL+'files/images/image.png" />';
				var errorClass = "";
					...
					if(self._validate(this.items[i].type) <= 0) {
					 ...
					 /* var fr = new FileReader;
						fr.onload = function() { // file is loaded
							...
							document.getElementById(uId).src = fr.result; // is the data URL because called with readAsDataURL
							
						};

						fr.readAsDataURL(this.items[i]); */
						 setupReader(this.items[i],uId);
					
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><img src="" id="'+uId+'" width="50" /><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="'+SITE_URL+'files/images/ajax-loader.gif" /></div></div>';
				}
			}
...
function setupReader(file,x) {
    var name = file.name;
    var reader = new FileReader();  
    reader.onload = function(e) {  
        var text = e.target.result; 
        document.getElementById(x).src = text;
    }
